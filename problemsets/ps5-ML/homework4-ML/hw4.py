...
def linreg_closed_form(train_X, train_Y):
	...
	Theta = numpy.linalg.inv(X_transpose.dot(train_X)).dot(X_transpose).dot(train_Y)
	y_preds = numpy.dot(train_X, Theta)
	#print("Theta : " , Theta, ", Mean squared error: %.2f" % mean_squared_error(train_Y, y_preds))

	MSE = numpy.square(numpy.subtract(train_Y, y_preds)).mean()
	print("Theta : ", Theta, ", Mean squared error: %.2f" % MSE)
	return Theta
...
def loss(Theta, train_X, train_Y):
	...
	Z = h_theta(Theta, train_X)

	# find the mean square loss
	rv = 1 / 2 * N * numpy.sum(numpy.square(Z - train_Y))
	return rv
...
def linreg_grad_desc(initial_Theta, train_X, train_Y, alpha=0.05, num_iters=500, print_iters=True):
	...
	for k in range(1,num_iters+1):
		...
		pred = cur_Theta[0] * train_X + cur_Theta[1]
		cost = (1 / m) * sum([val ** 2 for val in (train_Y - pred)])
		#print(" Iteration:", k, ", pred:", pred)
		md = -(2 / m) * sum(train_X * (train_Y - pred))
		bd = -(2 / m) * sum(train_Y - pred)

		cur_Theta[0] = cur_Theta[0] - md * alpha
...
if __name__ == '__main__':
	...
	init_theta = [0.00, 0.0]
	linreg_grad_desc(init_theta, data_X, data_Y, .01, 10, False)
